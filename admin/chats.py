...
def admin_chats():
    ...
    all_messages = models.ChatMessage.query.filter(
        (models.ChatMessage.sender_id == admin_user.id) | (models.ChatMessage.receiver_id == admin_user.id)
    ).all()
    ...
    users = models.User.query.filter(models.User.id.in_(user_ids)).all() if user_ids else []

    # حساب آخر رسالة لكل مستخدم لترتيب القائمة
    last_message_time = {}
    for msg in all_messages:
    ...
    if users:
        unread_counts_query = db.session.query(
            ...
            models.ChatMessage.receiver_id == admin_user.id,
...
def admin_chat_view(user_id):
    ...
    messages = models.ChatMessage.query.filter(
        or_(
            and_(models.ChatMessage.sender_id == user_id, models.ChatMessage.receiver_id == admin_user.id),
            and_(models.ChatMessage.sender_id == admin_user.id, models.ChatMessage.receiver_id == user_id)
        )
